    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['intel:attendance:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['intel:attendance:remove']"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("attend_state").then((response) => {
      this.statusOptions = response.data;
    });
    this.getDicts("type_status").then((response) => {
  ...
  methods: {
    ...
    /** 重置按钮操作 */
    resetQuery() {
      (this.time = ""), this.resetForm("queryForm");
      this.handleQuery();
    },
    // 多选框选中数据
