<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                label="操作">
                <template slot-scope="scope">
                <el-button v-if="isAuth('xuesheng','获得') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="jifenhuodeCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'获得':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','获得') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="jifenhuodeCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'获得':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','获得') && contents.tableBtnIcon == 0" type="success" size="mini" @click="jifenhuodeCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'获得':'' }}</el-button>
    ...
    <jifenhuode-cross-add-or-update v-if="jifenhuodeCrossAddOrUpdateFlag" :parent="this" ref="jifenhuodeCrossaddOrUpdate"></jifenhuode-cross-add-or-update>
...
import jifenhuodeCrossAddOrUpdate from "../jifenhuode/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      jifenhuodeCrossAddOrUpdateFlag: false,
...
//导出excel
  ...
  methods: {
    ...
    jifenhuodeCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.jifenhuodeCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.jifenhuodeCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.jifenhuodeCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
